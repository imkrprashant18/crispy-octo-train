 ...
 */
export async function updateDoctorStatus(formData: SetUserRoleFormData) {
        const isAdmin = await verifyAdmin();
        if (!isAdmin) throw new Error("Unauthorized");
 ...
 */
export async function updateDoctorActiveStatus(formData: SetUserRoleFormData) {
        const isAdmin = await verifyAdmin();
        if (!isAdmin) throw new Error("Unauthorized");

        const doctorId = formData.doctorId;;
        const suspend = formData.suspend === "true";
...
export async function approvePayout(formData: SetUserRoleFormData) {
        ...
        const payoutId = formData.payoutId;
        ...
        try {
                ...
                if (error instanceof Error) {
                        ...
                        throw new Error("Failed to approve payout: Unknown error");
                }
        }
}
